...
/// <summary>
/// Detects RSI oversold condition (RSI below threshold).
/// Label = true when RSI is below oversold level (default 30), indicating potential reversal up.
/// </summary>
public class RsiOversoldDetector : IPatternDetector
{
    public string PatternName => "RsiOversold";
    public int DefaultLookback { get; }

    private readonly int _rsiPeriod;
    private readonly double _oversoldThreshold;

    public RsiOversoldDetector(int lookback = 20, int rsiPeriod = 14, double oversoldThreshold = 30.0)
    {
