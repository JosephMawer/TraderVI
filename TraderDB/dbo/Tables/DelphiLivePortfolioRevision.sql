...
(
    ...
    [SettingsAssignmentId] UNIQUEIDENTIFIER NULL,
    ...
    CONSTRAINT [FK_DelphiLivePortfolioRevision_Settings] FOREIGN KEY ([SettingsAssignmentId]) REFERENCES [dbo].[EngineStrategyAssignment] ([AssignmentId]),
    CONSTRAINT [CK_DelphiLivePortfolioRevision_Identity] CHECK
        (ISJSON([SnapshotJson]) = 1 AND (([Revision] = 0 AND [LeaseId] IS NULL AND [LeaseFencingToken] IS NULL AND [SettingsAssignmentId] IS NULL) OR
         ([Revision] > 0 AND [LeaseId] IS NOT NULL AND [LeaseFencingToken] > 0 AND [SettingsAssignmentId] IS NULL) OR
         ([Revision] > 0 AND [SettingsAssignmentId] IS NOT NULL AND [LeaseId] IS NULL AND [LeaseFencingToken] IS NULL)))
);
GO
