...
7. **Outcome feedback is collecting but not mature.** ADR-0020 through ADR-0026 define the evidence, outcome, coverage, policy-separation, initial three-session swing marks and excursions, and promotion contracts. The additive schema matches TraderDB. Two official validation runs and one exploratory replay were captured over the same 2026-08-21 completed market session, so they count as one independent cohort. Athena now distinguishes immature horizons from terminal invalid session joins, reports definition-specific coverage, and can persist cost-aware 1/2/3-session marks plus MFE/MAE paths for published picks, but has not written an outcome because no future eligible session has matured. Track progress in `Docs/calibration-implementation-checklist.md`.
