...
- **Paper monitor**: Shared Core service used by TraderVI console and WPF. It persists delayed TMX receipts/evidence, evaluates the ADR-0028 policy, updates ghost positions, and may record an ADR-0031 database-only ghost exit.
- **Sentinel**: Reserved future always-on/live-risk boundary. It is not the current WPF process, and no live broker execution is authorized.
