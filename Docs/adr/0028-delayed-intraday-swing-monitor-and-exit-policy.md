...
### Accepted intraday evidence resolution

Persist completed five-minute OHLCV bars in the later version-1 intraday evidence ledger. Use TMX's direct completed fifteen-minute bar for the operational policy decision; when all three exact completed five-minute components exist, their deterministic aggregate must match the direct bar. A missing five-minute component is not synthesized because the source does not distinguish no trading from unavailable evidence. The accepted monitor still polls every fifteen minutes. One-minute bars remain a possible later research source, not a version-1 storage requirement. The durable collector/outcome design must version how a direct policy bar and its finer stored evidence are reconciled when a component is absent or disagrees.
